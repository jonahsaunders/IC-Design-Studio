"""Actual ngspice signed source-current and differential-voltage checks."""
...
def main():
    ...
    for typ,analysis,expected in [('op',{},.9),('tran',{'step':'10n','stop':'1u'},.9),('ac',{'start':'10','end':'1meg','points':10},.5),('dc',{'source':'V1','dc_start':'1','dc_stop':'-1','dc_step':'-.1'},-.5)]:
        q=clone(p);t=create(q,b['id'],typ);t['analysis']={'type':typ,'corner':'nominal','temperature':27,**analysis};t['probes']=['out','input'];t['measurements']=[{'name':'difference','kind':'voltage','node':'input','reference':'out','min':expected-1e-8,'max':expected+1e-8},{'name':'source_current','kind':'current','source':'V1','min':(abs(expected) if typ=='ac' else -expected)/10000-1e-10,'max':(abs(expected) if typ=='ac' else -expected)/10000+1e-10}];q['testbenches']=[t];validate(q);save_project(q,out/(typ+'.icproj'));r=simulate(q,t,args.ngspice,out/typ);rows.append({'analysis':typ,'status':r['measurements']['status'],'expected_voltage':expected,'measured_voltage':r['measurements']['measurements'][0].get('value'),'measurements':r['measurements']['measurements'],'samples':len(r['x'])});(out/'report.json').write_text(json.dumps(rows,indent=2))
