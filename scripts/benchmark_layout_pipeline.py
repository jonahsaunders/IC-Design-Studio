    ...
    try:
        for count in args.sizes:
            ...
            for i in range(args.samples):
                ...
                samples.append({**stages,'commit_total_ms':commit_ms,'first_paint_ms':paint_ms,'recovery_reopen_ms':read_ms,'live_checks_ms':check_ms,'complete_with_checks_ms':commit_ms+paint_ms+check_ms,'check_issue_count':len(check['issues']),'incremental_stats':check.get('stats',{}),'edit_stats':getattr(w.history,'layout_stats',{})})
        ...
        report={'version':__version__,'workflow_hash':WORKFLOW_SOURCE_HASH,'host':{'platform':platform.platform(),'python':platform.python_version(),'qt_platform':app.platformName()},'workloads':rows,'scope':'Full Studio connected move, synchronous durable recovery, first paint and live declared-rule/terminal checks. Timing stages can be nested; use totals rather than summing stage columns. No foundry deck or native Windows claim unless run there.'}
