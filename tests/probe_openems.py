...
from icstudio import inductor, openems_backend as backend, em_technology
from icstudio.model import save_project
from test_inductor_pdks import fixture


def main():
    parser = argparse.ArgumentParser(); parser.add_argument('--python', required=True)
    parser.add_argument('--out', type=Path, required=True); parser.add_argument('--mesh-check', action='store_true')
    args = parser.parse_args(); p = fixture(); tech = p['pdk']
    ...
    columns = {}; commands = []; environment = {**os.environ, 'MPLBACKEND': 'Agg'}
