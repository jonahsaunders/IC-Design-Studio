...
def main():
    ...
    d.tabs.setCurrentIndex(0);QTest.qWait(50);d.grab().save(str(out/'sources.png'))
    row=d.run();wait();assert row['state']=='Complete',row['log']
    assert d.wave.data and d.wave.data['timescale']=='1ps'
    assert 'Current revision' in d.summary.text()
    assert not w.jobs,'Digital payload was sent to the analog plot'
    d.wave.cursor=165000;d.wave.update();QTest.qWait(80);d.grab().save(str(out/'waveforms.png'))
    d.files.setCurrentRow(0);d.editor.insertPlainText('// New revision\n');assert d.apply()
    assert 'earlier revision' in d.summary.text()
    ...
    d.tabs.setCurrentIndex(0);d.files.setCurrentRow(0)
