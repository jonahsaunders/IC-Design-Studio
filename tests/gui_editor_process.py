...
p,cid,key=reference(tech('sky130A'));generate_mirror(p,cid);w.set_project(p);w.cid=cid;w.mode_combo.setCurrentIndex(1);w.refresh(True);w.set_keyboard_profile('Virtuoso-inspired',{});w.navtabs.setCurrentIndex(2);w.results_dock.hide();w.resizeDocks([w.nav,w.inspector],[360,290],Qt.Horizontal);w.layer_combo.setCurrentText(ls['m1']);w.start_layout_tool('path');w.editor_net.setCurrentText('IREF');w.layout.fit();QTest.qWait(100);w.grab().save(str(out/'mirror-editor.png'));w.cancel_tool();w.resize(1100,760);QTest.qWait(100);w.grab().save(str(out/'compact-editor.png'))
