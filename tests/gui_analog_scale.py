...
def workloads(include_ledger=False):
    ...
    if include_ledger:
        p = clone(p)
        p['cells'][0]['electrical'] = {'version': 1, 'nets': []}
        yield ('synthetic-large-identities', validate(p), c['id'], {
            'origin': 'Generated by tests/gui_analog_scale.py',
            'description': 'The same 500-device/10000-shape workload with persistent terminal and net identities.',
            'qualification': 'Synthetic identity-preserving edit workload; excludes process verification.'})
...
def main(argv=None):
    ...
    parser.add_argument('--command-path', choices=('generic', 'interactive'), default='generic',
        ...
        for name, project, edit_cid, description in workloads(args.electrical_ledger):
            ...
            with MemorySamples() as memory:
                for iteration in range(args.iterations):
                    ...
                    def edit():
                        if args.command_path == 'interactive' and hasattr(studio, 'capture_transform'):
                            assert studio.capture_transform([device_id], dx=20, label='Scale qualification move') == [device_id]
                        else:
