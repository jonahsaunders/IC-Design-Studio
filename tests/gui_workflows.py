...
p=example('empty');child=recipe_cell('ring',{'kind':'guard_ring','width':3000,'height':3000,'thickness':300});p['cells'].append(child);inst={'id':uid(),'name':'I1','cell':child['id'],'x':0,'y':0,'rotation':0,'nx':2,'ny':1,'dx':5000};p['cells'][0]['layout_instances']=[inst];w.set_project(p);w.mode_combo.setCurrentIndex(1);w.refresh(True);QTest.qWait(100);assert len(w.layout.cell['shapes'])==8;w.select([inst['id']],'layout');w.move([inst['id']],100,200,'layout');assert w.cell['layout_instances'][0]['x']==100;w.rotate();assert w.cell['layout_instances'][0]['rotation']==90;w.undo();w.undo();assert w.cell['layout_instances'][0]['x']==0
