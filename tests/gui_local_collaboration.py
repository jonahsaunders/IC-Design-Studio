"""Run the same local-host acceptance used by installed desktop builds."""
import argparse
import json
...
def main():
    ...
    from icstudio.local_collaboration_probe import run
    ...
    window = Studio(recover=False)
    window.maybe_save = lambda: True
    window.show()
    report = dict(status='failed')
    try:
        report['checks'] = [run(window, out)]
