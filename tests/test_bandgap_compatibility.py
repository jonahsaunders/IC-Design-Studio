...
class BandgapCompatibilityTests(unittest.TestCase):
    def test_reduced_program_preserves_all_non_program_source_bytes(self):
        folder=ROOT/'examples/gf180-bandgap';meta=json.loads((folder/'compatibility-source.json').read_text())
        reduced=(folder/'5vfullv2-compatibility.sch').read_text()
        ...
        original=(folder/'5vfullv2-original.sch').read_text().replace('C {devices/code_shown.sym} 2050 -680','C {devices/code_shown.sym} 2040 -720')
        self.assertEqual(hashlib.sha256(original.encode()).hexdigest(),meta['source_sha256'])
        self.assertEqual(hashlib.sha256(reduced.encode()).hexdigest(),meta['reduced_sha256'])
        def without_program(text):
            start=text.index('value="',text.index('{name=NGSPICE'))+len('value="');end=text.index('\n"}',start)
            return text[:start]+'<PROGRAM>'+text[end:]
    ...
    def test_six_case_program_imports_offline_and_catalog_formulas_survive_exchange(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            text=Path(exported['directory'],exported['top']).read_text();text=text.replace("ad=\"'int((nf+1)/2) * w/nf * 0.18u'\"",'ad="9e-12"')
            Path(exported['directory'],exported['top']).write_text(text)
