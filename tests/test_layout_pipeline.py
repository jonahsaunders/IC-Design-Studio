...
class HierarchicalScene(unittest.TestCase):
    ...
    def test_large_overview_is_bounded_and_zoom_query_stays_exact(self):
        ...
        self.assertTrue(rows[0]['_overview']);self.assertTrue(scene.stats['detail_reduced']);self.assertLessEqual(scene.stats['query_rows'],12000)
        self.assertEqual(rows[0]['id'],'array');self.assertEqual(scene.stats['query_rows'],0)
