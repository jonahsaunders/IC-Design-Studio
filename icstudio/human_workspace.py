...
class HumanWorkspaceMixin(GridSettingsMixin):
    ...
    def make_task_ribbon(self):
        ...
        specs = {
          ...
          'layout': [
            ('Draw', [('Select',lambda:self.set_tool(0),'select','select'),('Rectangle','Rectangle','rect','rect'),('Polygon','Polygon','polygon','polygon'),('Path','Path','path','path'),('Via','Place via','via','via'),('Cell','Place physical cell…','chip','instance_place')]),
