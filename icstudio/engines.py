...
def execute(args,cwd,timeout=180,input_text=None):
    proc=subprocess.Popen([str(x) for x in args],cwd=cwd,stdin=subprocess.PIPE if input_text else subprocess.DEVNULL,stdout=subprocess.PIPE,stderr=subprocess.STDOUT,text=True,encoding='utf-8',errors='replace',shell=False,start_new_session=os.name!='nt')
    prior=None
    try:
...
def run_ngspice(p,cid,settings,executable,directory,progress=lambda *_:None):
    ...
    directory=Path(directory).resolve();deck=directory/'input.cir';raw=directory/'result.raw';atomic_write(deck,preload(p,spice(p,cid,settings,hierarchical=False),directory));atomic_write(directory/'runtime-lock.json',json.dumps({'osdi':verified(p)},indent=2));progress(.05,'Starting ngspice batch worker')
    if raw.exists():raw.unlink()
    try:log=execute(ngspice_command(p,executable,raw,deck),directory)
    ...
    return {'schema':1,'created':now(),'engine':'ngspice (installed executable)','engine_hash':file_digest(exe_path),'project_id':p['id'],'revision':p['revision'],'design_hash':design_digest(p),'pdk_hash':digest(p['pdk']),'rule_hash':digest(p['pdk']['layers']),'cell_id':cid,'settings':settings,'x':xs,'x_label':{'tran':'Time (s)','dc':'Source value','ac':'Frequency (Hz)','noise':'Frequency (Hz)','op':'Operating point'}[typ],'y_label':'Noise (V/√Hz)' if typ=='noise' else 'Voltage magnitude (V)' if is_complex else 'Voltage (V)','traces':traces,'phase':phase,'operating_point':{},'warnings':['Locked technology model bindings are used when configured; otherwise generic level-1 models apply. External AC points are points per decade.'],'log':log}
