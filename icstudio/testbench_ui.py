...
def editor(studio,original=None):
    ...
    def add(m=None):
        if m is None and kind.currentText()=='noise':
            dlg.diagnostics.kind.setCurrentIndex(dlg.diagnostics.kind.findData('noise'))
            m={'name':'measure'+str(table.rowCount()+1),'kind':'input_noise'}
        ...
        for col,key in enumerate(keys):
            if key=='kind':
                ...
                w=QComboBox();w.addItems(['frequency','delay','range','voltage','current',*MEASUREMENTS]);w.setCurrentText(m[key]);table.setCellWidget(row,col,w)
